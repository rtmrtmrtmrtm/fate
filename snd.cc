                ...
                void *userData)
{
  ...
  const short int *buf16 = (const short int *) input;
  const int *buf32 = (const int *) input;
  assert(sizeof(int) == 4);
  ...
  for(int i = 0; i < (int) frameCount; i++){
    if(((wi + 1) % sin->n_) != ri){
      int s;
      if(format == paInt16){
        s = buf16[i*sin->channels_ + sin->chan_];
      } else {
        s = buf32[i*sin->channels_ + sin->chan_];
      }
#ifdef __FreeBSD__
      if(s >= 0x7fff0000){
        sin->nclip_ += 1;
        if(now() >= sin->last_clip_ + 5){
          fprintf(stderr, " CLIP-%d-%d-%d\n", sin->card_, sin->chan_, sin->nclip_);
          fflush(stderr);
          sin->last_clip_ = now();
          sin->nclip_ = 0;
        }
...
CardSoundIn::start()
{
  ...
  // most sound cards yield 16 bits, but if the O/S does
  // rate reduction there might be a few more useful
  // low-order bits. not, however, on FreeBSD.
  // so it's probably best to use rate=48000 in order
  // to control how rate reduction is done.
  format_ = paInt32;
